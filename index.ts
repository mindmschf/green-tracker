...
function readProductsFromFile(): { name: string; url: string }[] | null {
...
async function fetchProductLinks(): Promise<{ name: string; url: string }[]> {
    ...
    return Array.from(products.entries()).map(([url, name]) => ({ name, url }));
}
...
async function checkStockStatus(product: { name: string; url: string }): Promise<boolean> {
    try {
...
async function sendGroupedTelegramMessage(productsInStock: { name: string; url: string }[], timestamp: string) {
    if (productsInStock.length > 0) {
        ...
        const productList = productsInStock
            .map((product, index) => `${index + 1}. <a href="${product.url}">${product.name}</a>`)
            .join('\n');
...
async function main() {
    const now = new Date();
    const timestamp = now.toLocaleString('en-GB', { dateStyle: 'full', timeStyle: 'long', timeZone: 'Asia/Singapore' });
    console.log('main called', timestamp);
    const jstHour = now.getUTCHours() + 9;
    
    if (jstHour >= 7 && jstHour < 23) {
        const products = await readProductsFromFile();
        const productsInStock: { name: string; url: string }[] = [];
